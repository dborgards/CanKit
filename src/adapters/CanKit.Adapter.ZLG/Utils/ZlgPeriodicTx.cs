    ...
    private unsafe void ApplyHardware(bool enable, CanFrame frame, TimeSpan period)
    {
        ...
        if (frame.FrameKind is CanFrameType.Can20)
        {
            var obj = new ZLGCAN.ZCAN_AUTO_TRANSMIT_OBJ
            {
                ...
                index = _index,
                interval = interval,
                obj = frame.ToTransmitData(_bus.Options.TxRetryPolicy, _bus.Options.WorkMode == ChannelWorkMode.Echo)
            };
        ...
        else if (frame.FrameKind is CanFrameType.CanFd)
        {
            var obj = new ZLGCAN.ZCANFD_AUTO_TRANSMIT_OBJ
            {
                ...
                index = _index,
                interval = interval,
                obj = frame.ToTransmitFdData(_bus.Options.TxRetryPolicy, _bus.Options.WorkMode == ChannelWorkMode.Echo)
            };
